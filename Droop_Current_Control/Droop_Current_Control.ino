...
double K=0;                                    //droop coeficient initially set to 0
double dK;                                     //droop coeficient incriment
double Kp = 0.001;                             //Proportional coeff for the calculation of dK
double P;                                      //Power [W]
double V_ref = 80;                             //No load voltage [V]
...
V_DG = analogRead(1);
V_DG = map(V_DG,0,1023,0.1,100);              //maping starts from 0.1 to avoid deviding by 0 later on

V_out = analogRead(2);
V_out = map(V_out,0,1023,0,400);

P = analogRead(3);
P = map(P,0,1023,0,5000);

if(P==0){                                     //Avoid deviding by 0
  K = 0;
}
else{
  dK = (1-I_meas*V_DG/P)*Kp;
  K = K+dK;
}
